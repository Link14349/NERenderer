...
public:
    explicit HypersphereManifold(float radius = 1) : r(radius) {
        opencl.loadProgram("./HypersphereManifold/findGeodesic.cl", findGeodesic);
        opencl.loadProgram("./HypersphereManifold/getPoint.cl", getPoint);
        findGeodesic.loadKernel("geodesic");
        getPoint.loadKernel("getPoint");
    }
